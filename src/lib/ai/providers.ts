...
export async function callAiProvider(
  ...
  if (provider === 'deepseek') {
    ...
    if (!key) return { error: 'DEEPSEEK_API_KEY nije postavljen na serveru' }
  ...
  if (!key) return { error: 'GEMINI_API_KEY nije postavljen na serveru' }
...
export function buildContentPrompt(task: string, context: string): AiMessage[] {
  return [
    {
      role: 'system',
      content:
