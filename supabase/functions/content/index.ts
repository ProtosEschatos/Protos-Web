...
type ContentType = 'services' | 'portfolio' | 'blog' | 'testimonials' | 'pricing' | 'process'

const tableMap: Record<ContentType, { table: string; orderCol: string; filter: Record<string, unknown> }> = {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let query = supabase.from(mapping.table).select('*').eq('language', lang).order(mapping.orderCol)

    if (mapping.filter) {
      Object.entries(mapping.filter).forEach(([key, value]) => {
